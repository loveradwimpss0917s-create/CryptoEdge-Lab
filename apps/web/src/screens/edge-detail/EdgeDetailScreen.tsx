...
export function EdgeDetailScreen() {
  ...
  const transition = useMutation({
    mutationFn: (to_status: string) => api.transitionEdge(edgeId, to_status, "manual transition from UI"),
    onSuccess: () => {
  ...
  if (error || !data) return <p className="text-reject">Failed to load edge.</p>;
  ...
  return (
    <div className="max-w-3xl space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="rounded bg-slate-800 px-2 py-0.5 text-xs">{edge.status}</span>
      ...
      <section className="space-y-2 rounded border border-slate-800 bg-slate-900 p-4">
        <div>
          <h2 className="text-sm font-medium text-slate-400">Hypothesis</h2>
          <p>{edge.hypothesis}</p>
        </div>
        <div>
          <h2 className="text-sm font-medium text-slate-400">Rationale</h2>
          <p>{edge.rationale}</p>
        </div>
        {edge.counter_evidence && (
          <div>
            <h2 className="text-sm font-medium text-slate-400">Counter-evidence</h2>
            <p>{edge.counter_evidence}</p>
          </div>
        )}
        {evidence.length > 0 && (
          <div>
            <h2 className="text-sm font-medium text-slate-400">Evidence</h2>
      ...
      {data.current_version && (
        <section className="rounded border border-slate-800 bg-slate-900 p-4">
          <h2 className="text-sm font-medium text-slate-400">Current Version</h2>
