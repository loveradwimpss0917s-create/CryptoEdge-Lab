...
| `coinglass_v4` | CoinGlass (freemium) | 清算集計, OI 横断, L/S, ヒートマップ(有料) | 無料: 低頻度/主要指標 | API キー登録が必要 (非対話セッションでは取得不可) のため **未着手**。`okx_rest` (下段) を key-free な代替として先行実装 (docs/15 SONNET-1, 2026-07) |
| `okx_rest` (long/short・清算) | OKX v5 `rubik`/`public` | `/api/v5/rubik/stat/contracts/long-short-account-ratio-contract` (全アカウント L/S 比率), `/api/v5/public/liquidation-orders` (約定済み清算) | 認証不要 | **`long_short_ratios`/`liquidations_5m` の初のライブ書き手** (schedule.ts `makeOkxLongShortRatioAdapter` / `makeOkxLiquidationsAdapter`, tick-1h, 2026-07〜)。rubik エンドポイントは long%/short% の内訳ではなく単一比率 (long件数/short件数) を返すため、`ratio = long%/short%` かつ `long%+short%=1` から `long% = ratio/(1+ratio)` を逆算し `ratio_type='all_account'` として格納。OKX には「top trader」区分の公開比率が無いため `ls_top_trader_z_30d` は当面 DATA_PENDING のまま。清算は `sz` (契約枚数) に OKX 公表の契約額面 (BTC-USDT-SWAP=0.01 BTC, ETH-USDT-SWAP=0.1 ETH) を掛けて USD 建て名目を算出。`liquidation-orders` は直近の一部のみを返す (深いページングなし) ため、2 回のポーリング間隔を超えるバーストは恒久的に欠損しうる — docs/09 §3 liq-cascade-rebound 行が元々明記する「清算系列の不完全性」と同種の限界。**本番不具合修正 (2026-07)**: `instType=SWAP` に対して `instId` をフィルタとして渡すと HTTP 400 (本番の初回 tick-1h 実行で確認)。`instId` フィルタは `instType=MARGIN` 専用で、SWAP/FUTURES/OPTION は `instFamily` (例: `BTC-USDT-SWAP` → `BTC-USDT`) を使う必要があった。**続報 (同日 2 回目の tick-1h)**: `instFamily` 修正後は HTTP 400 は解消したが `group.details is not iterable` で新規に失敗 — 同一 instFamily 内で当該インスツルメントに約定が無いグループは `details` を空配列ではなく省略して返すことが判明。`group.details ?? []` で対応 |
...
| `etherscan` / `tronscan` | Etherscan/Tronscan (free key) | USDT Treasury ミント/バーン検知 (031)。対象アドレスの大口 Transfer 監視 | 5 req/s (free)。イベント → `events(usdt_mint)`。**Ethereum 側実装済み** (`workers/ingest/src/adapters/etherscan.ts`, 2026-07 レビュー TASK-4): Treasury アドレスへの zero-address からの Transfer (= mint) を検知。`ETHERSCAN_API_KEY` 未設定時は fail soft (0 件・スキップ)。Tronscan (Tron 側) は未実装 |
...
| `yahoo_finance` | Yahoo Finance (非公式) | NQ=F, ES=F, GC=F, BTC=F (CME), ^VIX 日中 | 非公式ゆえ冗長系必須 (Stooq CSV をフォールバック)。ToS リスクは docs/10 R-D2。**cme_gap 検知は実装済み** (`workers/ingest/src/adapters/yahoo-finance.ts`, 2026-07 レビュー TASK-4): BTC=F 日足の直近2本の日付が2日以上離れていれば (週末/祝日クローズ)、その間のギャップ率を `events(cme_gap)` へ記録。他系列 (NQ=F 等) は未実装 |
...
| `econ_calendar` | 手動シード + 公式ソース | FOMC (Fed 公式カレンダー年次固定), CPI/NFP (BLS スケジュール), Deribit 限月 SQ (計算可能), 半減期 | 年 1 回の手動更新 + ingest 自動補完。`events` へ。**投入機構は実装済み** (`workers/ingest/src/adapters/econ-calendar.ts`, 2026-07 レビュー TASK-4)。**2026 FOMC 8件は投入済み** (docs/15 SONNET-6, 2026-07: 複数の独立ソース — Fed公式の各会合ページ・fedratecalc.com・MEXC・financecalendar.com・Yahoo Finance — が一致する日付のみ採用)。CPI/NFP/PPI は BLS の年間全日程を検証できなかったため意図的に空のまま (2ヶ月分のみ確認できたが、残りを捏造しない方針)。投入されるまで無害な 0 件書き込み |
