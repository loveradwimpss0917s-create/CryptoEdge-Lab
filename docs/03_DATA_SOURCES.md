...
| `econ_calendar` | 手動シード + 公式ソース | FOMC (Fed 公式カレンダー年次固定), CPI/NFP (BLS スケジュール), Deribit 限月 SQ (計算可能), 半減期 | 年 1 回の手動更新 + ingest 自動補完。`events` へ。**投入機構は実装済み** (`workers/ingest/src/adapters/econ-calendar.ts`, 2026-07 レビュー TASK-4) だが `ECON_CALENDAR` は意図的に空配列で出荷 — サンドボックスから公式カレンダーを検証できないため、日付を捏造せず運用者が Fed/BLS の公式ソースから確認して投入する前提。投入されるまで無害な 0 件書き込み |
