...
| S-03 | イベント履歴バックフィル (cme_gap/usdt_mint/fomc過去) | なし | **cme_gap実行済み (410件, 2019-01〜)**。usdt_mintは`ETHERSCAN_API_KEY`未設定で保留。fomcは日程未確定でブロック中 (下記) |
...
| S-95 | FULL_DONE の「次のアクション」文言がverdictに関わらず常に「VALIDATEDを判断」固定でREJECT時に誤誘導 | なし | verdict別に文言分岐、ユーザー報告の実Edgeで確認 |
| S-96 | PAPER→ACTIVE ゲートもS-94と同一原因 (`sharpe`と`sharpe_bootstrap`の取り違え) で通過不能だった (先行発見・予防修正) | なし | 修正・テスト追加済み。本番でPAPER到達Edgeがまだ無いため実害は未発生のうちに対処 |
| S-97 | UIUX監査: Board カードが docs/06 §3 の設計 (score/verdict/試行数を常時表示) を実装しておらず、Edge を1件ずつ開かないと状態が分からなかった | なし | `GET /edges` に latest_verdict/latest_score/trial_count を追加、Board カードに表示 |
| S-98 | UIUX監査: Action Queue が docs/06 §1「ゼロインボックス型」の中核 (承認/却下ボタンをキュー内に直接表示) を実装しておらず、全項目がDossierへのクリックスルーのみだった | なし | 承認itemに[承認][却下]、dq itemに[解決]ボタンをキュー内に追加 |

**V1 完了宣言 = S-01〜S-07, S-91〜S-98 完了 + §2 V1表 全行 ✅**
...
### Phase RS: 研究OS化 (docs/21, 2026-07-12 設計レビューで新設 — **R2 の未着手分より優先**)

> 根拠: 本番実測で律速が「アダプタ本数」ではなく「1試行あたりコスト」(spec供給 0.8件/日)
> と確定したため (docs/21 §0)。カード詳細は docs/19 Phase RS 節が正。

| Task | 内容 | 依存 |
|---|---|---|
| S-100 | features parquet に fwd_ret 列 (EEP 規約再利用) | なし |
| S-101 | Feature Catalog (description + feature_stats/IC + API/UI) | S-100 |
| S-102 | Spec Builder GUI (BoolExpr ツリーエディタ) | S-101 |
| S-103 | literature Pack v2 + 取込API + Import Studio | S-101 |
| S-104 | edge_dossier / improvement Pack (on-demand 決定論生成) | なし |
| S-105 | Readiness Advisor チップのリンク化 | S-101 |
| S-106 | Signal Lab タブ1: Feature ランキング (IC) | S-100 |
| S-107 | Signal Lab タブ2: 条件ワークベンチ + spec 昇格 | S-102, S-106 |
| S-108 | Discovery Stage 1 (条件走査 + BH-FDR) weekly | S-100 |
| S-109 | Findings Inbox + finding_review Pack + IDEA昇格 | S-108 |
| S-110 | eval_metrics metric/segment 名の契約テスト (S-94/96 再発防止) | なし (随時) |
...
| Task | 内容 | 依存 |
|---|---|---|
| S-27 | ~~Discovery Stage 1~~ → **S-108 に統合** (docs/21) | — |
| S-28 | ~~Findings Inbox~~ → **S-109 に統合** (docs/21) | — |
| S-29 | CUSUM 劣化検知 + decay_investigation Pack 自動生成 | paper/live実績 (docs/21 で凍結指定) |
| S-30 | 相関・ポートフォリオタブ | ADOPT 発生後 (docs/21 で凍結指定) |
...
> **2026-07-12 追記**: 本節の構想は docs/21 (Research OS 構想) が Phase RS1〜RS5 として
> 具体化・置換した。1 (regime-conditional verdicts) と 2 (cross-instrument) は
> docs/21 §7 の Phase RS5 に収容済み。本節は歴史的経緯として残す。
