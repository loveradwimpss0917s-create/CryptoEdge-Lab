...
**screen run の「簡易」の実体** (2026-07 レビュー TASK-5): `run_kind='screen'` は `eval/pipeline.py` の `SCREEN_EEP_CONFIG` (permutation 200 回・bootstrap 300 回・walk-forward 3 fold、full run の 1000/2000/5 の 1/5〜1/7) で実行され、CANDIDATE→TESTING ゲートが読む `overall.ev_bps`/`overall.p_perm` の計算コストを実際に下げる。以前は `run_kind` に関わらず常に full 相当の設定で実行しており、「screen」は名目上のラベルに過ぎなかった。
