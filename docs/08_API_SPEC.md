...
| GET `/actions` | **実装済み** (docs/15 SONNET-7): Action Queue。`{items: [{kind: "approval"\|"review"\|"dq", edge_id, title, detail}]}`。findings由来の項目はDiscovery Engine未実装のため対象外 (V2) |
...
**実装状況 (docs/15 SONNET-8, 2026-07)**: 両エンドポイントとも実装済み。カタログはキー・サイズ・アップロード日時のみ返す —
スキーマは DuckDB-WASM 側の `DESCRIBE SELECT * FROM read_parquet(...)` で都度取得する設計 (サーバ計算なし、docs/01 §3.3) なので、
カタログ自体にスキーマ/期間を持たせる必要がない。

| Method Path | 説明 |
|---|---|
| GET `/lake/{path}` | **実装済み**: R2 curated/features Parquet の認証付きパススルー (**Range リクエスト対応済み** — DuckDB-WASM の部分読みに対応)。immutable キャッシュヘッダ (docs/15 SONNET-8) |
| GET `/lake/catalog` | **実装済み**: `{datasets: [{key, size, uploaded}]}` — curated/・features/ 配下の R2 キー一覧 (docs/15 SONNET-8) |
